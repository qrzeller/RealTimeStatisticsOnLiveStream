...
\begin{longtabu} to \textwidth {l >{\bfseries}X[r, 1] X[4] l}
	...
	& OpenCV & Une librairie BSD, init. Intel : Tout est a programmer sois-même. Très puissant car permet de traiter la vidéo image par image. Elle est une énorme librairie pour traitement d'image, etc... \newline Programmation : C, C++, Python, Mathlab...
	\newline Prix : Gratuit.  & Difficile \\
	
	& Wowza & Wowza est un serveur de contenu multimédia spécialisé dans la distribution de vidéos. La partie transcription est quand à elle principalement déléguée à des librairies tierces tel que FFmpeg. \newline Programmation : XML/fichier de configuration. \newline Prix : 65\$/mo (logiciel), 4500\$ (matérielle)  & Facile \\
	
	& Wowza transcoder & Est un module de wowza et permet la transcription des vidéos et principalement l'ajout d'overlay type images. Elle utilise une librairie propriétaire propre à Wowza \newline Programmation : Java \newline Prix : Compris par Wowza server & Moyen \\
	...
	& FFmpeg \& Libav & Programme de transcodage puissant mais très limité dans l'intégration de contenu visuel.  & Facile \\
	
	& Red5 & Programme de transcodage puissant mais très limité dans l'intégration de contenu visuel.  & Facile \\
	...
	& Microsoft IIS Media Services & Programme de transcodage puissant mais très limité dans l'intégration de contenu visuel. Ce serveur ne doit plus être utilisé comme solution de service multimédia. Son support s'est terminé avec Windows Server 2008. Quand bien même une "extended lifetime" est prévue jusque en 2020. \cite{IIS_media} & Ne pas utiliser \\
	
	& Mist server & Serveur de distribution multimédia et de transcription (non live). Très modulaire, rapide et a empreinte CPU-Mémoire faible. Idéal si l'on ne veut pas investir dans une solution hardware car se place au delà des concurrents de type serveur web.  \newline Language : C++ (Aussi le programme en lui-même.)\newline Prix : Gratuit (OpenGL, sans DRM), Commercial : 2500\$ & Moyen \\
